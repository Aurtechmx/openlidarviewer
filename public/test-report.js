...
function build(){
  ...
  const r=[
    ...
    "SETUP",`Name or alias: ${val(fd,"name")}`,`Email: ${val(fd,"email")}`,`Role / affiliation: ${val(fd,"role")}`,`OLV version or link: ${val(fd,"olvVersion")}`,`Operating system: ${val(fd,"os")}`,`Browser + version: ${val(fd,"browser")}`,`Reference software: ${val(fd,"reference")}`,`Reference software version: ${val(fd,"referenceVersion")}`,`Dataset: ${val(fd,"dataset")}`,`Registered dataset id: ${val(fd,"datasetId")}`,`Independent of the project: ${yes(fd,"independent")}`,"",
