...
- A scan with no resolved linear unit no longer states metric figures it cannot measure. The USGS 3DEP density-floor comparison, the ground-return density, the complexity window and radius in ground metres, and the metre RMSEz, NVA and VVA on a contour map sheet were all derived from source coordinates through a placeholder factor of 1. A sheet printed "vertical unit unverified" for its contour interval and a metre RMSEz three rows below it. The figures are withheld now, and the complexity summary says the pts/m² density threshold could not be applied rather than producing no caveat, which read as clearing it.
