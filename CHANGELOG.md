...
- `src/main.ts` goes from 5,557 lines to 4,977 and `src/render/Viewer.ts` from 6,423 to 6,223, with the Analyse panel at 2,911. The module graph holds 841 modules and no cycles. The scan route, the Viewer-to-Inspector visual synchronisation and the streaming panel controls each moved into a coordinator over narrow ports, the action registry is split into contributors and built lazily, the Viewer's render core is built in a bootstrap module, and the Analyse panel's raster and relief previews moved into a surface-tiles module. Fan-out on the concentration modules is watched and printed as a table: 112, 77 and 23.
...
- Contours export as a package with a DXF, a validation record and the Contour Studio settings. A scientific artifact passport, an evidence boundary inspector and a SHA-256 over the DTM surface are implemented and tested but not reachable from any user path; they are listed in `docs/validation/unreachable-modules.json`.
...
- A stockpile preview read "± 41 m³ (1σ) · High confidence", the vocabulary of a calibrated interval. Its own claim register approves an exploratory preview with spatial correlation and base uncertainty unquantified: the sample term assumes independent thickness observations where returns are spatially correlated, and the base term is a heuristic spread that is zero for an explicit base. The band is named a model band and the grade states what it measures, the sampling.
- A TPI slope-position class read a non-finite slope as zero, which is the value that separates flat from middle-slope, so a cell whose slope never resolved could be labelled flat. It stays nodata.
- A lasso reclassify into a class the class filter was hiding landed the edit and hid its own result in the same frame, so the tool read as inert. The target class is revealed and the toast says the filter was opened. The user's other hidden classes stay hidden, and the derived-provenance caption survives the edit.
...
- No product changed evidence level. The register holds 34 claims: 2 at E1, 6 at E2, 9 at E3 and 17 at E4, with none at E5. The E5 work this cycle registers the Rogue tiles, makes the dev and holdout split deterministic and exposure-honest, and recomputes manifest invariants. The holdout has not been run.
...
- Five products reach `E4_CROSS_IMPLEMENTATION_VALIDATED` this cycle, taking the register from twelve to seventeen products at E4. Each was checked against an independent implementation on frozen synthetic or analytic fixtures; none is field accuracy against a surveyed reference, and no product reaches E5.
- `CRS-UTM-PROJECTION` reaches `E4_CROSS_IMPLEMENTATION_VALIDATED` as a new claim. A WGS-84 latitude and longitude projected to a WGS-84 UTM zone, hemisphere, easting and northing was checked against GeographicLib 2.7 and PROJ 9.8.1 over 36 frozen geodetic fixtures. Zone and hemisphere matched on all 36, including the widened Norway zone 32V and the Svalbard zones 31X, 33X, 35X and 37X; easting and northing agreed with both to under 1.5 mm, the largest differences being 4.531e-4 m in easting and 9.521e-4 m in northing. PROJ and GeographicLib agree with each other to 2.0e-9 m, so the residual is attributable to the candidate (study `CRS-UTM-PROJ-GEOGRAPHICLIB-THIRTYSIX-FIXTURES`). The datum is WGS 84 on both sides, so no datum transformation, PROJ grid, vertical reference or ECEF conversion is exercised; latitudes outside 80S to 84N are refused; and agreement with two implementations is not accuracy against a surveyed monument.
- `CHANGE-RASTER` reaches `E4_CROSS_IMPLEMENTATION_VALIDATED` from E2. Per-cell change was checked against GRASS 8.5.0 `r.mapcalc` over eight synthetic epoch pairs under protocol `PROTO-CHANGE-GRASS-MAPCALC`. Both sides select the same 3007 comparable cells, including a pair with holes in one epoch, and the largest relative difference is 7e-6 under a 1e-5 gate derived from Float32 storage (study `CHANGE-RASTER-GRASS-MAPCALC-EIGHT-PAIRS`). Only one-metre metre-CRS square-cell grids were compared; agreement on synthetic epoch pairs is not accuracy against surveyed field change; per-cell LoD95 is pending and co-registration error is absent by construction.
- `CHANGE-VOLUME` reaches `E4_CROSS_IMPLEMENTATION_VALIDATED` from E2. Integrated cut and fill volume was checked against GRASS 8.5.0 `r.univar` over the same eight synthetic epoch pairs, and against the closed-form integral each pair was built from, agreeing with both to within 1e-6 (study `CHANGE-VOLUME-GRASS-MAPCALC-EIGHT-PAIRS`). The same scope holds as for the raster, and change-volume uncertainty stays exploratory.
- `HOLDOUT-RMSE` and `NVA-VVA` reach `E4_CROSS_IMPLEMENTATION_VALIDATED` from E3. The internal holdout accuracy statistics were recomputed in base R 4.4.1 over six frozen residual vectors: bias, RMSE and max absolute residual agree to about 4.4e-16, and the 95 percent figure (1.96 times the pooled RMSE) agrees to about 8.9e-16 (studies `ACCURACY-RMSE-R-RESIDUALS` and `ACCURACY-NVA95-R-RESIDUALS`). The R leg establishes the computation, not accuracy: it confirms RMSE is the raw second moment and not a standard deviation, and that the multiplier is applied correctly. These are internal holdout diagnostics, so the held-out points share the source of the interpolated ones; this is not independent checkpoint accuracy and not ASPRS NVA/VVA compliance, and both claims still require E5. Median, NMAD and P95 are excluded from the R comparison because R interpolates quantiles at type 7 while the code takes the nearest rank.
...
- Contours draw over the point cloud in the 3D scene as a derived layer, with panel controls for visibility, index emphasis, opacity and vertical lift. Solid and dashed support differ by colour and alpha rather than dash pattern, and a segment crossing a data gap is not drawn, so a drawn line never spans unmeasured ground. Re-analysis regenerates the layer in place and closing its source scan removes it. Each layer carries the digest of the analysis receipt that produced it, built through the provenance path exports already use. Receipts for the other analytical products need analysis-time provenance and are not included.
- An opt-in resident-stickiness pass for the streaming selector, enabled with `?stickiness=on`, which holds an already-resident node against a marginally better competitor to reduce swap churn at the point-budget boundary. A node with a finer candidate beneath it in the same tick competes on its raw score with no bonus, so retention cannot stall refinement. The default is unchanged and reproduces the v0.6.5 selection exactly; the option is not yet measured in a browser.
- A Work workspace mode that gathers the scene-work tools in one place, an Output panel that consolidates every export behind one surface, and a slimmer Streaming panel whose saved views moved to the Inspector. Process Studio reports live product status and can be docked from the command palette.
- Layer groups in the Layers panel. A group is a named, collapsible set of layer ids with visibility, solo, rename, collapse, select and a member picker. Each action writes through the existing `LayerService` calls one layer at a time, so a group never holds a second copy of visibility. Membership persists by stable layer id and is additive within session schema v8, so files written before groups existed load unchanged.
- An inspection issue workflow on annotations: a severity (low, medium, high, critical), an open or resolved status, and an observation date, with an issue list ordered worst first and a roll-up of what is still open. Resolved reads "marked resolved" throughout, because the viewer records the status and does not verify that anything was fixed. Severity is carried by a glyph and a word as well as by colour.
- One Speed to Quality performance control in the top bar, with an Auto position and an Advanced disclosure that keeps every underlying knob individually settable. It drives the streaming preset, the resident point budget, concurrent decodes, the device-pixel-ratio ceiling, Eye Dome Lighting and antialiasing. It does not touch the static load budget, which voxel-reduces the cloud that terrain analysis, measurement and export all read, so no slider position changes the computation applied to a given point set. On a streamed scan it does change the resident-point budget, so a resident-only measurement, analysis or export reads whatever is resident when it runs.
- A project-wide elevation colour scale, opt-in and applied per frame, so two scans in one project can share a range instead of each stretching its own.
- Krovák (EPSG:5514) ingest, iPhone PDRF7 reading, and footprint reprojection for any proj4-defined projected CRS rather than UTM alone. The Krovák definition uses a seven-parameter Bursa-Wolf shift, which agrees with authoritative PROJ to about 5 cm where the naive three-parameter shift is roughly 10 m out.
- Parsers for 3D Tiles `tileset.json` and PNTS, a local out-of-core read planner, and a cross-CRS project placement planner.
...
- A derived-layer model and store, a feature-extraction service over the building and conductor cores, a scientific receipt serializer over the analysis record, and a product executor registry: a foundation that invokes a registered compute core through `ProcessService.runIfAuthorized`, preparing one fail-closed execution seam. Production product routes do not pass through it yet, so it constrains nothing on its own in this release.
...
- The `TPI` and `VRM` terrain descriptors reach `E4_CROSS_IMPLEMENTATION_VALIDATED`, splitting the former combined `VRM-TPI` claim. Each is checked three ways against an independent tool AND the closed form on an analytic fixture designed to control the confound: `TPI` against gdaldem 3.13.1 over 3364 interior cells to under 9×10⁻⁷ within a 1×10⁻⁵ tolerance, on a low-amplitude quadratic (so the reference's float32 mean accumulation stays below tolerance); `VRM` (Sappington et al. 2007) against SAGA 7.8.2 over 3136 interior cells within a 1×10⁻⁴ tolerance, on a smooth tilted quadratic (so Horn's normal and SAGA's estimator converge), reproducing the closed-form Sappington VRM to under 1×10⁻⁹ (studies `TPI-GDALDEM-ANALYTIC` and `VRM-SAGA-ANALYTIC`).
- The cross-section profile (`MEAS-PROFILE`) reaches `E4_CROSS_IMPLEMENTATION_VALIDATED`, its required level. No single tool exposes a corridor percentile over a point cloud, so the reference is assembled from two: OGR/SpatiaLite 5.1.0 places every point on the section line (`ST_Line_Locate_Point`, `ST_Distance`) and R 4.4.1 `quantile(type = 7)` reduces each station, with the corridor query passing elevations through as text so neither side re-prints a number the other reads. Over 751 stations across three parameter sets the largest difference is 3.6×10⁻¹⁵ m against a 1×10⁻⁶ m tolerance registered before the reference existed, per-station corridor counts match exactly, and the four deliberately empty stations read as gaps on both sides. One of the two fixtures also carries a closed form, because ten corridor elevations in an exact arithmetic progression reduce without a sort, and both sides reproduce it exactly (study `MEAS-PROFILE-OGR-R-CORRIDOR`). Both fixtures are synthetic and hold every point clear of a bin boundary and of the corridor edge, so the tie-breaking there is untested and nothing here is accuracy against a surveyed section.
- `CHANGE-VOLUME` reaches `E2_ANALYTICALLY_VERIFIED`. Cut/fill volumes are checked against integrals solved on paper over analytic surface pairs: parallel planes at a fixed separation (area times separation, bit-exact), a plane tilted about the footprint centroid (same volume, since cell-centre quadrature is exact for a linear surface), a quadratic dome whose cell-centre sum and true integral differ by the closed-form defect k·L²s²/6 (so halving the cell size quarters the error), a cone against πR²H/3 and against its own level-of-detection truncation, a saddle whose lobes cancel, and a surface differenced against itself. The oracle enters at the grid pair, so it covers the integration and not the DTM build feeding it.
- `UNCERTAINTY-BAND` reaches `E2_ANALYTICALLY_VERIFIED`. The band is checked against the scaling laws its documented error model implies rather than against its own formula: the random term grows as the square root of the changed-cell count while the systematic co-registration term grows linearly with it, so relative error falls as 1/√N under survey noise and does not move at all under a registration bias. A single cell that just clears the level of detection sits exactly at the band's 95% detection threshold in volume terms, tying the two modules to one convention.
- E57 ingestion (`E57-INGEST`) reaches `E4_CROSS_IMPLEMENTATION_VALIDATED`. The decoded cartesian coordinates, namespaced surface normals and colour of a public CC-BY terrestrial scan were compared against PDAL 2.10.2 `readers.e57` over all 1,788,994 points and nine dimensions, as an exact quantised integer sum at a 1×10⁻⁶ quantum: a single point wrong by one micrometre fails the comparison instead of being absorbed into a mean. Every dimension agreed exactly (study `E57-INGEST-PDAL-CROSS-DECODE`). Intensity sits outside the comparison because PDAL rescales it by 65535/(max − min) without subtracting the minimum, so the two sides report different numbers for the same file. The scan is a CloudCompare re-export rather than a scanner-native write, so vendor extension blocks, multi-scan files and spherical coordinates are untested. Twelve products are now at E4.
- The terrain and measurement algorithms changed in this cycle. What changed is stated in Fixed below: the scan-shape aspect denominator, the reported dimensions of a scanned space, unit handling across the measurement and classification chains, the ICP overlap refusal, the change-volume band, and the USGS Quality Level floor. The contour generalization mode added above is opt-in and off by default, so contour geometry is unchanged for every existing purpose unless the mode is selected. The interactive surfaces added this cycle are listed in Added above.

### Fixed

- The USGS 3DEP Quality Level table read a QL3 density floor of 0.25 pts/m². The published floor is 0.5, so a survey between the two was awarded QL3 at half the required density. QL0, QL1 and QL2 were already correct. Datasets in that band now read below-QL3.
- Hiding the navigation legend took the view controls with it. Top, Front, Side, Iso and the orthographic toggle exist nowhere else in the app, and the dismissal is persisted, so hiding the help text removed them permanently and across sessions. The legend and the view rows are now separate regions.
- A cluster of unit-correctness defects: WKT2 `LENGTHUNIT` is parsed for linear and vertical units; the measurement chain scales vertical and volume quantities by the vertical unit; SMRF ground tolerances and classifier parameters are physical and converted at the boundary; camera flatness and project-card sizes read height off the up axis.
...
- The reported L x W of a scanned space came from a rectangle whose direction was taken from the covariance eigenvectors of the footprint. A square room and a round one have no dominant direction, so sampling noise chose it and the rectangle drifted toward the diagonal: an 8 by 8 metre room read between 8.51 and 11.31 metres across twenty samplings of the same geometry, and a 29 by 14 room read 15.25 wide. Both sides now come from the smallest rectangle enclosing the footprint, the same one the scan-shape routing measures, so the panel and the router describe one rectangle and a room reads the same on every scan of it. The square room reads 8.00 and the 29 by 14 room reads 14.00.
...
- Streaming corrections: in-flight decodes get eviction hysteresis and the budget counts decoded-pending nodes; a partial stream stays resident-only beside a static cloud; the streaming panel no longer reads ready with decoded-pending nodes.
- The Clip panel is reachable on mobile, the app lands on Data mode before render rather than after `openScan` resolves, and the layer Remove button is no longer overlapped by the workspace scrollbar.
- One font policy across all panels, enforced by a lint, with the Analyse and Streaming panels snapped to the type-scale tokens.
- A two-epoch change volume with no error budget at all is no longer reported as certain. Both terms of the band are built from caller-supplied numbers, so a level of detection of 0 with no co-registration RMSE collapsed the band to ±0, which cleared every threshold in the module and graded the least supported result available as detectable at 0% relative error and high confidence. An empty budget now reports as unquantified, and the compare panel says the ±0 band bounds nothing and names what to supply. A comparison with no changed cells is handled the same way.
- `ScanShape.aspect` divided the vertical extent by the longer side of the axis-aligned horizontal bounding box, which is a property of the source CRS axes rather than of the scan, so one scene read 0.17454 unrotated and 0.12446 at 45 degrees. Three comparisons against two thresholds route terrain on that value, so a scan near a threshold routed on how it happened to be oriented on disk. The denominator is now the longer side of the minimum-area rectangle enclosing the horizontal outline, which reproduces the axis-aligned extent on an axis-aligned footprint and gives 2r on a disc, so both thresholds keep their meaning. The value now holds through a sweep of angles and is exact on the four quarter turns. A hull of more than 512 vertices, which a densely sampled convex outline such as a silo wall or a scanner's outer ring produces, has the rectangle scan measure a uniformly spaced subset of it, and that subset is anchored to the hull's first vertex, so the measured size can still move by up to 1.9e-5 under rotation there; at or below 512 vertices the hull is measured whole. Every fixture keeps its kind, spaceKind, nonTerrain, up and confidence, and the largest aspect movement is 2.0 per cent. The `extent` field is untouched and still follows the axes.
- A projected CRS whose GeoTIFF keys carry no ProjectedCSCitationGeoKey (3073) was named from the systematic WGS 84 UTM ranges or, failing those, from the bare EPSG code, so a file on another datum lost the datum from its name: `utm15.las`, written by libLAS 1.2, read as `EPSG:26915` rather than as UTM zone 15N on NAD83. The projected display name now resolves in order: 3073, then the curated registry label for the EPSG code, which carries the datum, then the systematic WGS 84 UTM naming that covers all 120 zones including the ones the registry does not list, then GTCitationGeoKey (1026), then the bare `EPSG:<code>`. That file now reads `NAD83 / UTM zone 15N (EPSG:26915)`. 1026 ranks below both code-derived names because it guarantees only free text for the CRS the file is in, so a 32615 file whose 1026 read `NAD83` would otherwise print a geographic datum name over a metre grid. A WKT VLR outranks all five, an empty or whitespace-only 1026 falls through, and a geographic CRS still uses its own geographic citation.
- The LAS and LAZ stride sampler drew its jitter across the full step and clamped an overshoot onto the last record, so the final short bucket over-weighted that record: at count 11 with step 3 it took two thirds of the bucket's probability rather than one third, and at count 10 with step 100 it was drawn about nine times in ten. The draw is now across the records the bucket holds, so every record in every bucket is equally likely.
- An E57 CompressedVector holding zero records was refused. libE57Format closes such a section with an 8-byte data packet that declares no bytestreams and carries no length table, which failed the check that a packet declares one bytestream per prototype field. A packet declaring no bytestreams in a section declaring no records now ends the walk and every prototype column decodes to length 0. A packet that declares bytestreams still has to declare one per prototype field, and a section that declares records still has to supply them.
- EPT attribute widths were checked against one set of 1, 2, 4 and 8 bytes without reference to the declared type, so a field declaring `float` at 1 or 2 bytes reached the unsigned-integer branches of `readAttr` and returned an in-range wrong number with no error; on X, Y or Z that is a coordinate that looks valid. IEEE-754 has no 1-byte or 2-byte binary format, and widths are now keyed by type: `signed` and `unsigned` take 1, 2, 4 or 8, `float` takes 4 or 8. A present but malformed `scale` or `offset` was also treated as absent, so a manifest declaring a scale of 0.01 as a string decoded that axis at scale 1. Absent still takes the decoder default; present must be a finite number.
- Three 3D Tiles fail-opens. A `boundingVolume` declaring more than one of box, region and sphere produced a merged object, so the tile took whichever form the code read first; the declared key count is now checked before per-shape validation. A tile declaring a `refine` that names no strategy the specification defines silently took its parent's value, and a non-string `refine` raised a TypeError out of `toUpperCase` rather than a parse error; both go through one helper now, and an absent `refine` still inherits. A malformed `RTC_CENTER` in a PNTS feature table was downgraded to no centre, which placed a georeferenced tile at its local origin because POSITION is tile-local; it must now be three finite numbers, and absent still reads as no centre.
- `window.__OLV_TEST_API__` was mounted behind a runtime check on `?test=1` alone, so every shipped bundle carried it. Same-origin policy stops another page reading it, but a script already on the viewer's origin gained a stable surface that can place measurements and rewrite classifications, and those figures reach an exported integrity report whose digest signs them. The seam is now gated on a build-time constant, true for the dev server and for a build run with `OLV_TEST_SEAM=1`, which is what the Playwright webServer sets. Every other build substitutes false and the minifier drops the block, so the string `__OLV_TEST_API__` appears in no file under `dist`, and `?test=1` there parses as before and mounts nothing.
- `IDENTITY`, the base of every `TransformStore` stack, was exported unfrozen, so `readonly` bound at compile time only and a cast could redefine the original placement for every layer at once. It is frozen through the rows, since freezing the outer object leaves them writable.
