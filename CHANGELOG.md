...
- The terrain and measurement ALGORITHMS are inherited from v0.6.5, apart from the opt-in contour generalization mode above, which is off by default. What did change in this cycle is stated in Fixed below: unit handling across the measurement and classification chains, the ICP overlap refusal, the change-volume band, and the USGS Quality Level floor. The application around them did not stand still either; the interactive surfaces added this cycle are listed in Added above.
