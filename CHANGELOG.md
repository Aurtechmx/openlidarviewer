...
- A lasso reclassify into a class the class filter was hiding landed the edit and hid its own result in the same frame, so the tool read as inert. The legend is recounted from the edited classification and the target class revealed, and the toast says the filter was opened. The counts were stale after any lasso edit: only the derive and fill paths recounted them.
