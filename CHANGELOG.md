...
- **Analytical/cartographic geometry split.** Exact analytical isolines and a generalized cartographic product that references the analytical hash, records displacement statistics and a topology flag, and is never labelled exact. Terrain-aware generalization scales the per-feature tolerance by support, confidence, closure, and length within a bounded band.
