...
v0.6.4 hardens reliability and data integrity, and tightens precision. It hardens how remote scans stream and how they cancel and time out, makes session and export writes snapshot-consistent, and turns a large class of "unknown coordinate unit" cases from a silent number into a refusal or a disclosed caveat. It tightens the terrain surface and the measurement readout against known synthetic references. It also lifts several blocks out of the two monoliths, splits the stylesheet into ordered sections, and adds opt-in classification with export improvements. The measured figures in this document come from the release-mode gate run at the tagged commit.
...
- a scan that already carries a ground classification builds its terrain surface from those ground points directly, instead of re-deriving ground with the SMRF filter. The filter dropped legitimately steep bare-earth returns and interpolated across the gap; trusting the classification keeps every ground node as a measured cell. On a bare-earth reference grid the surface goes from 0.27 m node error with 2,742 interpolated cells to the single-precision storage floor with every cell measured, which also splits a ridge-side contour the interpolated surface had bridged. The behaviour engages only when the cloud is fully ground-classified; a raw or partly-classified cloud still runs the filter unchanged;
- the measurement readout is precise enough to show what it measures. A two-millimetre distance read `0.00 m` before, because each such figure rounded to two decimals; they now carry five significant figures with a per-unit bound, so a small separation reads in millimetres and a long baseline keeps the millimetre place while staying legible.
...
- the loader, the measurement path and the terrain pipeline were checked against three synthetic calibration datasets whose exact values are known, generated by A Urias. Every point count, class count and return count read back exactly, together with the bounds and the CRS. Measured distances, pole heights, plane slope and footprint area match their known values. The terrain surface reproduces the reference elevation range exactly, and its contour levels match the reference and agree with GDAL's independent contour output on the reference grid. These are agreements with analytic synthetic references, not a field survey, and they carry no product-to-product claim. Registering the terrain and contour cross-checks into the evidence register is a follow-up.
