...
| `olv.terrain.evidence.sensitivity` | 1 | DTM model sensitivity raster, written only on request: per-cell highest minus lowest height over a fixed four-member ensemble (canonical, ground filter slope 0.15, slope 0.2, inverse distance weighting void fill) and the number of members with a height; describes model sensitivity, not accuracy | internal (ensemble recorded in `validation/protocols/evidencedem-sensitivity-ensemble-v1.md`) |
| `olv.terrain.evidence.residual` | 1 | DTM leave-local-out reconstruction residual: each measured cell held out and rebuilt by the canonical geodesic fill from its measured 8-neighbours; every measured cell up to 250,000, otherwise every k-th; shows how well neighbours predict a cell, not accuracy | internal (rules recorded in `validation/protocols/evidencedem-attention-v1.md`) |
| `olv.terrain.evidence.attention` | 1 | DTM attention raster: level 0 to 3 and one dominant reason per cell; each input normalised to 0 to 1 by a fixed rule, level is the highest score banded at 0.33, 0.67 and 1, no weighted blend; written only on request; the reason code table is part of this version; describes where to look, not accuracy | internal (rules recorded in `validation/protocols/evidencedem-attention-v1.md`) |
