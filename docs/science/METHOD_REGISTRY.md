...
| `olv.terrain.evidence.attention` | 1 | DTM attention raster: level 0 to 3 and one dominant reason per cell; each input normalised to 0 to 1 by a fixed rule, level is the highest score banded at 0.33, 0.67 and 1, no weighted blend; describes where to look, not accuracy | internal (rules recorded in `validation/protocols/evidencedem-attention-v1.md`) |
