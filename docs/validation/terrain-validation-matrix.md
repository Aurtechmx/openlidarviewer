...
is validated". **Slope, aspect, hillshade and contours are the products at E4+** (slope, aspect and hillshade each
independently cross-implemented against GDAL 3.13.1 on the same analytic fixture, and contours against GDAL `gdal_contour`
on a frozen analytic tilted plane); every other product tops out at E3 synthetic known-truth,
