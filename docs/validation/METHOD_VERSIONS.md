...
- `olv.contour.generalize.dp@1`: a Douglas to Peucker variant with a fixed tolerance, available in the geometry-product library.
- `olv.contour.generalize.terrain-adaptive@1`: a per-feature variant that scales tolerance by support, confidence, closure, and length within a bounded band, then applies the same Chaikin corner-cutting. This is the id stamped when a surface selects terrain-aware generalization, which reaches the shipped export path.
