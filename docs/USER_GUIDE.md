...
Open the Analyse tab. Its home lists Terrain, Flow Pulse, Terrain Access, Observatory and Objects & Space, each with its status and a one-line reason. A blocked row names what would lift it: Flow Pulse and Terrain Access need a terrain run first, so their rows offer Prepare terrain, which opens the Terrain page. Before a run the Terrain row also offers Run terrain analysis, which opens Terrain and starts the run. Once a run has made contours, a Contours row sits under Terrain and opens them in one click. After a run the Terrain status is the stricter of Process Studio's readiness and the run's own verdict, so a run that says it is not usable never shows as Ready. Range frames shows Scanner grid present, since no readiness check applies to it.
