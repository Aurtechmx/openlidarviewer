...
builds the seam. Read Parts 1 to 2 for what the application does now; read Parts
3 to 5 for where the analysis layer is meant to go.
...
The `PointSampler` contracts in Part 3 (C1 to C5) are not enforced today. There
...
3. Land the contract tests (C1 to C5) as real, non-skipped tests bound to the new
