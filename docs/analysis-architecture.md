...
The indicator copy is uniform across the project. See `src/analysis/coverageMessages.ts` for the canonical strings.
...
The worker implementation will live at `src/analysis/runtime/analysisWorker.ts` when a cpu-heavy analysis lands.
...
The "looks correct, missing half the data" failure on streaming clouds is a structural risk, not a feature gap. An analysis that iterates "the cloud" can silently see only the resident set and produce a confident-looking result against a fraction of the data. The contracts in §8 prevent that class of bug from shipping: each analysis must declare and surface its coverage, and the test for each contract becomes the gate.
