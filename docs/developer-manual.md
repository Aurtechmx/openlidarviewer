...
Once a scan is open you can navigate it in 3D, recolor it and measure it. The
Scan Intelligence report describes the dataset, and the result can be exported.
...
- `tests/releaseAssetVerifier.test.ts` builds its fixtures with the `zip` CLI,
  ...
  That leaves one boundary: **the Windows leg does not cover release-asset
...
As a rule, each `src/io/*` file owns exactly one format or one concern;
...
- Unit (Vitest, Node). The algorithmic core is test-first: format
  ...
  measurement core is covered too (geometry, formatting, serialization, label
  layout), along with the exporters and both analysis modules. Tests assert against deterministic
...
- End-to-end (Playwright). `tests/e2e/viewer.spec.ts` drives the built
  app. It loads a sample and confirms the cloud renders and the Scan Report
  appears, then loads a second cloud and drops an E57 scan. It also closes a scan
  and loads another, and checks embed mode. `tests/e2e/measure.spec.ts` covers the measurement
  toolbar with its kind picker and units toggle, then round-trips a distance
  placement through session export. `tests/e2e/rendering.spec.ts` toggles Eye Dome Lighting
