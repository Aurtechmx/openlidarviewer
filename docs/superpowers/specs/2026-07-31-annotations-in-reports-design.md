...
four PDF reports, and they do not reach the integrity report. A user places a
marker, exports a terrain report, and the marker is not in it.
...
because they are exactly what they are once the frame is named. A marker drawn
on a map sheet cannot: it asserts a place, and a place needs a transform.
...
- **Markers: only with a real frame.** No frame, no marker, and the sheet says
  so where the markers would have been.
...
each one separately is how "also do E57" becomes a fourth job, and how the COPC
and EPT paths drifted apart earlier.
...
action row as "Clear all" — they act on the same set, so they belong together.
They must not look alike: "Clear all" destroys work and PDF does not. Same size
and shape, different treatment, per the destructive-versus-constructive rule.
Two identical buttons side by side would say the actions are equivalent.
...
- `profilePdf` — annotations within the corridor only, as station offsets
  rather than plan positions, because the sheet is a section and a plan
  position has no place on it.
