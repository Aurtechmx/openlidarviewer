...
| Continuity history colour surface | `HistoryTargets` | From `resize` at one backing-store size until the next different size, or `dispose` | `HistoryTargets.dispose()`, and `resize` to a different size, which frees before allocating |
