...
| Application services | `src/app` | ~21k | Composition root and the services that own shared state. |
| UI | `src/ui` | ~37k | Panels, Inspector, Studio surfaces, onboarding. |
| Shell | `src/main.ts` | 4,689 | Wiring. A monolith under decomposition. |
...
`src/main.ts` (4,689): the largest blocks, which are the extraction
candidates:

`buildActionRegistry` is a thin assembler in `src/app/actionDefinitions.ts` over
seven contributors in `src/app/actions/` (camera, view, tools, analysis, export,
workflow, help), each taking only the collaborators its actions use; the shell
builds the registry on the first palette, sheet or Dataset Story use from a lazy
chunk. The candidates that remain:
...
v0.6.9 tree reads 2911 lines and fan-out 23 after the UI wiring that followed.
...
`src/render/Viewer.ts` (6,136): the constructor and a handful of large
methods dominate:

Done: the renderer, scene, cameras and EDL pipeline are built by
`src/render/viewerRenderBootstrap.ts` (which also holds the EDL output-node
builder) with the settings the constructor held; the sizing, pixel-ratio,
clip-range, background and depth-mode decisions are pure in
`src/render/renderBootstrapPolicy.ts` and Node-tested
(`tests/renderBootstrapPolicy.test.ts`). The Viewer keeps the fields and the
...
| Block | Lines | Extraction target |
|---|---:|---|
| `constructor` | 435 | controls, tools and listener wiring (view-bound remainder) |
