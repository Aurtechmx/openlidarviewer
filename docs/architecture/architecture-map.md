...
| Render | `src/render` | ~69k | three.js/WebGPU scene, streaming scheduler, measurement tools, colour modes. |
...
**`src/main.ts` (4,977)** — the largest blocks, which are the extraction
candidates:
...
**`src/render/Viewer.ts` (6,172)** — the constructor and a handful of large
methods dominate:
