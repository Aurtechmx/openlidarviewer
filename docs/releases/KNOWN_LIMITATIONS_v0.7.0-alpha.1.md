...
`src/main.ts` is 4,970 lines and `src/render/Viewer.ts` is 6,153, seventy below
...
caught an added line twice during this cycle, and a banked drop once. Fan-out is 109 for the shell, 76 for
the renderer and 23 for the Analyse panel, across 899 modules with no
dependency cycles.
