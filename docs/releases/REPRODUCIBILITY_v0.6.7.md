# Reproducibility: OpenLiDARViewer v0.6.7
...
| `test-evidence-v0.6.7.json` | attached to the release | **Yes**. Generated by the exact-tag run |
...
Test counts and the bundle size are not reproduced in this table. They live in `docs/validation/test-evidence.json` for a development run and in the attached `test-evidence-v0.6.7.json` for the release, and `npm run lint:evidence` holds the documents to them.
...
| SBOM (CycloneDX, root `0.6.7`, production scope) | `npx @cyclonedx/cyclonedx-npm --omit dev --output-file sbom.json` |
