...
`src/main.ts` goes from 5,557 lines to 4,977 and `src/render/Viewer.ts` from 6,423 to 6,223, with the Analyse panel at 2,911. The module graph holds 841 modules and zero cycles. Fan-out on the concentration modules is watched and printed as a table: 112 for the shell, 77 for the renderer, 23 for the Analyse panel.
