# Reproducibility — OpenLiDARViewer v0.6.1
...
what caught it. This release re-baselines the figures for the same reason, since
the three regression tests and the benchmark framework's own tests changed the
counts.
...
| `test-evidence-v0.6.1.json` | attached to the release | **Yes** — generated by the exact-tag run |
...
|---|---|
| unit 3,357 (16 skipped) · export 618 · terrain 1,245 · ui 429 · slow 533 — 6,182 passed / 16 skipped | `npm run test:unit` / `test:export` / `test:terrain` / `test:ui` / `test:slow` (the large buckets run sub-sharded — unit ×3, terrain ×2, slow ×2 — so a run prints per-shard totals that sum to the figures above) |
...
| Live entry 718 KiB (within the 720 KiB ceiling, 2 KiB of headroom, above the 680 KiB warning line) | `npm run build:live && npm run check:bundle` |
...
| SBOM (CycloneDX, root `0.6.1`, production scope, 59 components) | `npx @cyclonedx/cyclonedx-npm --omit dev --output-file sbom.json` |
