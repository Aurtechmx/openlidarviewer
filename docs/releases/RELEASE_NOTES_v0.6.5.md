...
## Coordinate systems and export

- the resolved coordinate system is authoritative wherever an export identifies or computes a frame: the DEM and contour `.prj`, the PDF report, the Visual Export Studio label and scale-bar unit, and file conversion all describe the CRS the user resolved, not the file's declared one. A scan resolved to local reports local, not the rejected CRS;
- the measurement exports (CSV, GeoJSON, the PDF report and the integrity report) read one shared measurement engine, so a distance, area, slope or volume reads the same value on screen and in every file, including on a Y-up scan and a compound metre-over-foot CRS;
- a scan whose linear scale is unverified labels its exported measurements as source units with a stated caveat rather than asserting metres.
...
- the initial EPT manifest read is bounded in size and time and wired to cancel, like every other remote read, so a hostile or stalled `ept.json` cannot over-allocate or hang the load;
- a streaming scan's coordinate system, provenance and usage become authoritative only after the new scene commits, through one seam shared by COPC and EPT, so a failed or cancelled swap leaves the previous scene, its CRS and its provenance intact;
...
- the remote object's identity is pinned across a load, with validators and total size checked on every read with `If-Match` where offered, so a file re-uploaded mid-decode fails with a distinct code instead of splicing two versions;
...
The complete list is in `KNOWN_LIMITATIONS_v0.6.5.md`. Carried forward from v0.6.4 and unchanged: a large class of terrain and measurement products still fail closed on an unconfirmed unit rather than guess; there is no cross-CRS reprojection into a common viewer frame; and the residual streaming flicker at the point-budget boundary is unchanged. New this cycle: multi-layer mounting is enabled, with one outstanding precision refinement (the renderer's far-apart-mount render-origin fold, bounded and refused past 1 mm by the mount-precision gate) and a dedicated pick/measure-on-a-mounted-tile browser assertion still to be written; the DTM checkpoint results are external agreement on found data, not a preregistered field campaign, and the forest checkpoint is N=1; the ground-filter agreement holds on low-relief terrain and diverges on steep terrain, which is out of scope; the feature-extraction and registration interactive UIs are staged, not shipped; the Visual Export Studio CRS label for a streaming scan reads that source's own coordinate system rather than a mid-session override; and the frozen validation snapshot under `validation/snapshot/` is the 0.6.2 validation baseline carried forward, because the cross-platform portability and defect-audit runs that would refresh it were not repeated this cycle (see `REPRODUCIBILITY_v0.6.5.md`).
