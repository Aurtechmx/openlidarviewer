...
Physical multi-layer mounting is DISABLED in v0.6.5 (`MULTI_LAYER_MOUNT_ENABLED = false`) while the per-layer frame fixes land, and the flag is guarded so the shipped state cannot drift on. Multiple layers may be loaded and analysed individually; they are not co-registered and are not merged into one estimator. The shared project frame and the compatibility model are present and tested, and the app owns a live project frame, but nothing places two layers into one mounted frame in this build, so two-layer placement is unverified in a browser by construction. Cross-layer comparison, shared clipping and cross-layer measurement do not yet read the frame's offsets.
