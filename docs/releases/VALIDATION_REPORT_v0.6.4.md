...
Three synthetic calibration datasets were used more broadly, each with known exact values. Point, class and return counts, the bounds and the CRS read back exactly. Measured distances, pole heights, plane slope and footprint area match their known values. The reference elevation range and the contour levels reproduce exactly, and the same range and levels agree with GDAL's independent contour output on the reference grid. This is agreement with analytic synthetic references and an independent contouring tool, not a field survey. It records the DTM and contour behaviour against an independent reference; it does not by itself move the formal evidence grade, which for the terrain products stays as recorded below. Registering these cross-checks into `src/validation/crossCheck.ts` and the claim register is a follow-up.
...
One classification behaviour changed, the terrain surface gained a ground-trust path, and evidence was added. Building classification now gates on ground support, and a synthetic evaluation shows it removing scan-edge void artifacts without dropping real buildings on fully measured ground. The ground-trust path takes the delivered surface to the storage floor on a bare-earth reference, checked against an independent reference and an independent contouring tool. The formal terrain and contour evidence state did not move: three products remain cross-implemented at E4 and the rest at E3, and registering the new cross-checks is a follow-up. Nothing here is field validation. The correct reading of v0.6.4 is that classification precision improved on a targeted synthetic failure mode, the report and export surfaces refuse unknown units instead of guessing, and the terrain record is carried forward as it stood.
