...
| B26 | READ ASPRS Table 9 note 3 | io, terrain | The specification says the Withheld bit should generally be set on legacy Overlap Points, since they are culled during flight-line merging. OLV discards Withheld, so on such a file the producer's excluded points would enter terrain, density and stockpile as ordinary returns. The discard is verified in code. The consequence is not measured: every LAS fixture in this repository is a synthetic PDRF 6 file with no flag bit set, so nothing here exhibits it. Grounding it needs a real producer file carrying withheld or overlap points. | B | unmeasured | READ | OPEN | |
...
| B28 | READ `clipCloud.ts`, `voxelDownsample.ts` | model | Classification flags survived a load but not a derivation. Clipping now carries them and a test pins the alignment. Voxel downsampling still drops them: it takes the first point in a voxel for every attribute, and applying that to a safety flag would let one withheld point among nine ordinary ones lose its marking. Nothing consumes the flags yet, so that aggregation is left undecided rather than guessed. EPT and COPC decode on their own path and produce no flags, which reads as absent rather than as false zeros. | B | med | READ | OPEN | PARTIALLY FIXED |
