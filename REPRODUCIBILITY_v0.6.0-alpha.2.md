...
|---|---|
| Unit 2,781 (16 skipped) · export 586 · terrain 1,210 (18 skipped) · ui 429 · slow 508 — 5,514 passed / 34 skipped | `npm run test:unit` / `test:export` / `test:terrain` / `test:ui` / `test:slow` (the large buckets run sub-sharded — unit ×3, terrain ×2, slow ×2 — so a run prints per-shard totals that sum to the figures above) |
