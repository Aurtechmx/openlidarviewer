...
|---|---|
| Unit 2,766 (16 skipped) · export 586 · terrain 1,207 (18 skipped) · ui 429 · slow 508 — 5,496 passed / 34 skipped | `npm run test:unit` / `test:export` / `test:terrain` / `test:ui` / `test:slow` (the large buckets run sub-sharded — unit ×3, terrain ×2, slow ×2 — so a run prints per-shard totals that sum to the figures above) |
