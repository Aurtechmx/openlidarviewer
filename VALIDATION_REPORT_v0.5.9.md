...
- Evidence gating. `tests/exportManifest.test.ts` verifies the export-decision resolver's downgrade-only invariant (a registered exporter is capped to exploratory when the launch context isn't fully supported, never upgraded). NOTE: the resolver is implemented and unit-tested but has no production caller in v0.5.9 — Contour Studio ships as a foundation shell with no functional file export, so this is a tested model, not yet a runtime-enforced gate; it becomes enforcing when those exports land. `tests/contourDeliverablePdfModel.test.ts` and `tests/contourPackageManifest.test.ts` verify a blocked product yields no polished deliverable, an exploratory product is watermarked, and no survey-grade / certified / standards-compliant wording is asserted.
