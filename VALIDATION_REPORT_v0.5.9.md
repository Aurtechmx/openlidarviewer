...
- Hold-out validation. `tests/holdoutRmse.test.ts`, `tests/spatialBlockHoldout.test.ts`, and `tests/dtmSurfaceModel.test.ts` verify the internal accuracy estimator on analytic surfaces and verify that the per-fold train-only reclassifier — now wired into the shipped analyser — excludes held-out points from ground classification, closing the classify-before-split leak; `tests/calibrateConfidence.test.ts` verifies confidence calibration is scored out-of-fold.
- Geometry split and generalization. `tests/contourGeometryProduct.test.ts` verifies analytical geometry is immutable under cartographic settings, cartographic geometry references the analytical hash, and gaps are not bridged; `tests/contourPurposeExportBytes.test.ts` verifies the purposes export pairwise-distinct geometry via a bounded per-purpose tolerance. `tests/contourAdaptiveGeneralize.test.ts` exercises the terrain-aware per-feature generalization module (tolerance scaled by support, confidence, closure, and length), which is available in the library but is NOT the shipped export path — the shipped cartographic export uses the uniform per-purpose tolerance.
- Evidence gating. `tests/exportManifest.test.ts` verifies the export-decision resolver's downgrade-only invariant (a registered exporter is capped to exploratory when the launch context isn't fully supported, never upgraded), and `tests/contourExportPermit.test.ts` verifies the single authoritative permit that mints each contour export through it. The resolver is ENFORCED in v0.5.9 for the Contour Studio exports: GeoJSON, DXF, SVG, the contour report PDF, the DEM raster package, the complete deliverable, and the terrain intelligence report are each minted through `resolveContourExportPermit` → the resolver, the writer refuses a blocked permit, and the decision (validated / exploratory + watermark) is stamped into the exported file's provenance. `tests/contourDeliverablePdfModel.test.ts` and `tests/contourPackageManifest.test.ts` verify a blocked product yields no polished deliverable, an exploratory product is watermarked, and no survey-grade / certified / standards-compliant wording is asserted; the multipage technical-report PDF and complete-ZIP emitters are wired to these models.
