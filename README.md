...
- [VALIDATION_REPORT_v0.6.5.md](docs/releases/VALIDATION_REPORT_v0.6.5.md): what this release validates and what it does not. The terrain and measurement algorithms are inherited from [VALIDATION_REPORT_v0.5.9.md](docs/releases/VALIDATION_REPORT_v0.5.9.md); this cycle widens the terrain evidence onto real airborne LiDAR, adds two absolute-accuracy checkpoint results, and moves contours to cross-implementation evidence against `gdal_contour`.
- [KNOWN_LIMITATIONS_v0.6.5.md](docs/releases/KNOWN_LIMITATIONS_v0.6.5.md): the documented limits of this release (multi-layer mounting disabled, georeferenced products fail closed on an unconfirmed unit, no cross-CRS reprojection, residual streaming flicker; the DTM checkpoint results are external agreement on found data rather than a preregistered campaign, the forest checkpoint is N=1, the ground-filter agreement is scoped to low-relief terrain, and the feature-extraction and registration UIs are staged, not shipped).
