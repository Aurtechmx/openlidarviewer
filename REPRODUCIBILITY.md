...
This page is the single entry point for reproducing OpenLiDARViewer's build, tests, and reported analysis figures. It stitches together the pieces that also live in `README.md`, `REVIEWER_QUICKSTART.md`, and `docs/benchmarks.md`. The exact figures reported for the current release, each tied to the command that regenerates it, are in [REPRODUCIBILITY_v0.7.0-alpha.1.md](docs/releases/REPRODUCIBILITY_v0.7.0-alpha.1.md).

## Environment

- Node.js 22.18.0, the canonical toolchain, pinned in `.nvmrc`. `package.json` `engines` is deliberately wider (`^22.18.0 || >=24.11.0`) so the package installs on newer runtimes; only `.nvmrc` fixes the version the published figures come from.
...
The authoritative deterministic gate is `OLV_GATE_MODE=release npm run gate`; read the verdict from its `GATE EXIT:` line. `npm run test:release` is the static stage that gate runs first (typecheck, lints, all test buckets, both builds, bundle budget, plus plain and live-obfuscated smoke); run on its own it prints no `GATE EXIT:` line and refreshes no evidence. The broader browser matrix that a device varies, the full end-to-end suite and the mobile responsive job, runs in CI (`.github/workflows/ci.yml`), not in `test:release`. The remaining physical-device coverage gaps for a tagged release are recorded in `docs/releases/RELEASE_NOTES_v0.7.0-alpha.1.md`.
