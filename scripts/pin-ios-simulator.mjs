 ...
 *
 * `pick-ios-simulator.mjs` exists because `ios-simulator.yml` used to run on
 * whatever iPhone generation `macos-latest` happened to ship, and naming one
 * there would go red the day the image rotated. This leg is no longer in
 * that position: L13 traces its crash to the host's paravirtual GPU
