#!/usr/bin/env node
/**
 * verify-scientific-records.mjs — one command that re-reads the scientific
 * evidence already committed to this tree.
 *
 * This is an ORCHESTRATOR OF VERIFIERS. Every check it runs already exists as
 ...
 *
 * THE NAME SAYS RECORDS BECAUSE IT ONLY READS RECORDS. This file used to be
 * called validate-scientific.mjs, and the `validate:scientific` script pointed
 * straight at it. That name promised more than the file delivers: nothing here
 * runs the classifier, the ground filter or the terrain field, so a producer
 * that had drifted from the record it once wrote would still be graded green.
 * `validate:scientific` is now the composition that runs the producing bucket
 * first and this verifier second, and it lives in package.json where the
 * release chain's ordering contract is already visible. Run this script alone
 * when the question is whether the committed records still verify, which is the
 * only question an extracted source archive with no test fixtures can answer.
 *
 ...
 *
 * Usage:
 *   node scripts/verify-scientific-records.mjs
 *   node scripts/verify-scientific-records.mjs --out <dir>
 */
...
function writeReports(outDir, run) {
  ...
  const jsonPath = join(outDir, 'scientific-records.json');
  writeFileSync(jsonPath, `${JSON.stringify(run, null, 2)}\n`);

  const csvPath = join(outDir, 'scientific-records.csv');
  ...
  const mdPath = join(outDir, 'scientific-records.md');
...
function markdown(run) {
  ...
  const lines = [];
  lines.push('# Scientific records verification report');
  lines.push('');
  lines.push(
    'Generated by `scripts/verify-scientific-records.mjs`, which orchestrates the existing ' +
      'verification scripts. It records what each committed check reports; it does not ' +
      'recompute any metric or move any claim. The command that recomputes first and ' +
      'then verifies is `npm run validate:scientific`.',
  );
  lines.push('');
...
function main() {
  ...
  if (argv.includes('--help') || argv.includes('-h')) {
    process.stdout.write(
      'Usage: node scripts/verify-scientific-records.mjs [--out <dir>]\n' +
  ...
  process.stdout.write('scientific records verifier (orchestrates existing verification checks; recomputes nothing)\n');
  ...
  const run = {
    tool: 'verify-scientific-records',
    schemaVersion: 1,
