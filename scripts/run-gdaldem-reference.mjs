...
import { writeFileSync, mkdirSync, readFileSync, existsSync, rmSync, realpathSync } from 'node:fs';
...
function main() {
  ...
  let resolvedPath;
  const which = run('sh', ['-c', `command -v ${GDALDEM}`]);
  try {
    resolvedPath = which.exitCode === 0 ? realpathSync(which.stdout.trim()) : 'unavailable: not on PATH';
  } catch {
