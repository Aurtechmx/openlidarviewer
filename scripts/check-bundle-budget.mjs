...
const BUDGETS = [
  { prefix: 'index', maxKiB: 730, warnKiB: 700 },   // live ~728 KiB. Hard ceiling lowered 800→720 at v0.6 P1 to lock in the AnalysePanel/ObjectPanel lazy-mount win (was 792); raised 720→724 at v0.6.5 for the compass CRS-frame truthfulness gate and the scan-footprint export wiring; raised 724→730 at v0.6.5 as a deliberate, committed step for the desktop workspace shell (DesktopWorkspace: the Data/Work/Analyse/Output rail that re-hosts the existing panels). Like the compass/footprint step this is EAGER composition wiring — the workspace shell is the immediate desktop chrome shown on first scan, not an optional heavy feature, so it belongs in the shell rather than a lazy chunk. The heavy panels it hosts (Analyse/Object) still ride their own lazy chunks. Warn 700 keeps flagging creep.
