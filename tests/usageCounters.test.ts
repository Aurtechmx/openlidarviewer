...
describe('usageCounters — basic increment + snapshot', () => {
  ...
  it('snapshot returns most-recently-used first', () => {
    // Pin EVERY increment under fake timers so the ordering does not
    // depend on the wall clock — a fixed-date fake plus a real-time
    // pre-increment used to invert this assertion whenever the host
    // clock crossed the fake date. Each increment lands at a strictly
    // later epoch so the LRU contract is exercised deterministically.
    vi.useFakeTimers();
    try {
      vi.setSystemTime(new Date('2000-01-01T00:00:00Z'));
      increment('scan-open', 'laz');
      vi.setSystemTime(new Date('2000-01-01T00:00:01Z'));
      increment('scan-open', 'copc');
      vi.setSystemTime(new Date('2000-01-01T00:00:02Z'));
      increment('scan-open', 'ply');
    } finally {
      vi.useRealTimers();
    }
