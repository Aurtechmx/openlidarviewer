...
test('the COPC streaming chunks load when a COPC file is opened', async ({ page }) => {
  ...
  // openStreamingCopc shows the streaming panel before its dynamic imports —
  // its appearance confirms the COPC branch was taken.
  await expect(page.locator('.olv-streaming-panel')).toBeVisible({ timeout: 15_000 });
