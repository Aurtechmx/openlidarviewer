...
test('remembers a settings change across a page reload', async ({ page }) => {
  ...
  // v0.3.8: the Fixed point-sizing chip lives inside the Rendering
  // collapsible section, which defaults closed for first-paint
  // density. Open it before reaching for the chip.
  async function openRendering(): Promise<void> {
    const d = page.locator('details.olv-section-collapsible', {
...
test('arrow keys orbit the camera in orbit mode', async ({ page, context }) => {
  ...
  const readShareLink = async (): Promise<string> => {
    // v0.3.10: "Share" was renamed to "Copy view link" — the link only
    // reproduces the camera angle, not the scan (local-first), so the
    // button label now says so explicitly.
