...
describe('skyPresets — radial-gradient backgrounds', () => {
  ...
  it('every definition uses a radial-gradient', () => {
    for (const id of SKY_PRESET_ORDER) {
      expect(getSkyDefinition(id).background).toContain('radial-gradient');
    }
  });

  it('the catalogue has exactly nine presets (5 inspection + 4 v0.3.7 polish)', () => {
    expect(SKY_PRESET_ORDER).toHaveLength(9);
  });
