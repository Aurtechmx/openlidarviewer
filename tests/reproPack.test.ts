 ...
 *   M5  terrain complexity: VRM (Sappington et al. 2007) is slope-independent —
 *       a constant 45° plane scores ~0 while an alternating rough surface scores
 *       high — and TPI (Weiss 2001) localises relief (analytic crest value +
 *       ridge class), each with the derived confidence envelope.
 */
...
describe('reproducibility + calibration', () => {
  ...
  test('M5: VRM is slope-independent; TPI localises relief (analytic fixtures)', () => {
    const makeGrid = (cols: number, rows: number, f: (r: number, c: number) => number) => {
      const z = new Float32Array(cols * rows);
      for (let r = 0; r < rows; r++) for (let c = 0; c < cols; c++) z[r * cols + c] = f(r, c);
      return z;
    };

    // (a) constant 45° plane, 20×20 — steep but perfectly smooth.
    const cols = 20;
    const rows = 20;
    const plane = makeGrid(cols, rows, (_r, c) => c);
    const dPlane = hornSlopeAspect(plane, cols, rows, 1);
    const vrmPlane = computeVRM(dPlane.slope, dPlane.aspect, cols, rows, { windowCells: 3 });

    // (b) alternating rough surface (triangle wave, period 4, ±1 m), 16×9 —
    // the same 45° faces, but decoherent normals.
    const wave = [0, 1, 0, -1];
    const rough = makeGrid(16, 9, (_r, c) => wave[c % 4]);
    const dRough = hornSlopeAspect(rough, 16, 9, 1);
    const vrmRough = computeVRM(dRough.slope, dRough.aspect, 16, 9, { windowCells: 3 });

    // (c) single ridge, 9×9 (z=10 on row 4), radius-1 TPI: crest TPI = +5
    // exactly (hand-computed), standardised > +1 SD → class ridge.
    const ridge = makeGrid(9, 9, (r) => (r === 4 ? 10 : 0));
    const tpiRidge = computeTPI(ridge, 9, 9, { radiusCells: 1, slope: hornSlope(ridge, 9, 9, 1) });
    const crest = tpiRidge.tpi[4 * 9 + 4];
    const crestClass = tpiRidge.classes![4 * 9 + 4];

    const slopeDeg = (Math.atan(dPlane.slope[10 * cols + 10]) * 180) / Math.PI;
    metrics.M5_terrain_complexity = {
      planeSlopeDeg: round(slopeDeg, 2),
      planeVrmMedian: round(vrmPlane.summary.median, 6),
      planeVrmIqr: round(vrmPlane.summary.iqr, 6),
      roughVrmMedian: round(vrmRough.summary.median, 6),
      roughVrmIqr: round(vrmRough.summary.iqr, 6),
      ridgeCrestTpi: round(crest, 4),
      ridgeCrestStdTpi: round(tpiRidge.stdTpi[4 * 9 + 4], 4),
      ridgeCrestClassIsRidge: crestClass === TPI_CLASS.ridge,
      envelopeConfidence: {
        planeVrm: vrmPlane.confidence,
        ridgeTpi: tpiRidge.confidence,
      },
    };
    // Slope-independence: 45° of steepness contributes ~0 ruggedness…
    expect(slopeDeg).toBeCloseTo(45, 6);
    expect(vrmPlane.summary.median).toBeLessThan(1e-12);
    // …while the rough surface is decisively above it, with dispersion reported.
    expect(vrmRough.summary.median).toBeGreaterThan(0.05);
    // TPI: analytic crest value and class.
    expect(crest).toBeCloseTo(5, 6);
    expect(crestClass).toBe(TPI_CLASS.ridge);
    // The envelope confidence is derived (data support < 100%), never a constant.
    expect(vrmPlane.confidence).toBeGreaterThan(0);
    expect(vrmPlane.confidence).toBeLessThan(100);
  });
});
...
afterAll(() => {
  ...
  const m5 = metrics.M5_terrain_complexity as {
    planeSlopeDeg: number;
    planeVrmMedian: number;
    planeVrmIqr: number;
    roughVrmMedian: number;
    roughVrmIqr: number;
    ridgeCrestTpi: number;
    ridgeCrestClassIsRidge: boolean;
  };
...
| M5 | VRM median ± IQR on a constant ${m5.planeSlopeDeg}° plane (slope-independence) | ${m5.planeVrmMedian} ± ${m5.planeVrmIqr} |
| M5 | VRM median ± IQR on the alternating rough surface (same 45° faces) | ${m5.roughVrmMedian} ± ${m5.roughVrmIqr} |
| M5 | TPI at a synthetic ridge crest (analytic truth +5) / classified ridge | ${m5.ridgeCrestTpi} / ${m5.ridgeCrestClassIsRidge ? 'yes' : 'FAIL'} |
...
shows the reported uncertainty band is calibrated rather than nominal. M5
evidences the terrain-complexity design choice: VRM (Sappington et al. 2007)
separates ruggedness from steepness — a constant 45° plane scores ~0 while a
rough surface with the same face slope scores high — and TPI (Weiss 2001)
reproduces the analytic crest value with the correct slope-position class.
`;
