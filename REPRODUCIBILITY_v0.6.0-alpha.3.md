...
|---|---|
| unit 3,077 (16 skipped) · export 605 · terrain 1,240 · ui 429 · slow 517 — 5,868 passed / 16 skipped | `npm run test:unit` / `test:export` / `test:terrain` / `test:ui` / `test:slow` (the large buckets run sub-sharded — unit ×3, terrain ×2, slow ×2 — so a run prints per-shard totals that sum to the figures above) |
