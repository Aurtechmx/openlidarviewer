...
- The evidence ladder (E0–E6) is defined in `docs/validation/EVIDENCE_MODEL.md`. Four products are at E4: the slope raster, the aspect raster and the hillshade were cross-implementation validated against GDAL 3.13.1 on one analytic fixture (see `tests/slopeCrossCheck.test.ts`, `tests/aspectCrossCheck.test.ts` and `tests/hillshadeCrossCheck.test.ts`), and the contour set against GDAL `gdal_contour` on a frozen analytic tilted plane (see `tests/contourCrossCheck.test.ts`). Every other product tops out at E3 (synthetic known-truth checks).
