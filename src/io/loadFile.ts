...
async function preflightFile(
  ...
  if (format === 'unknown') {
    if (is3dTilesName(file.name)) {
      throw new LoadError(
        'unsupported-format',
        `A 3D Tiles tileset opens by URL, not as a dropped file: the document ` +
          `names its .pnts tiles by relative path, and a file picked off disk ` +
          `has no directory to resolve them against. Paste the tileset.json URL ` +
    ...
    worker.onmessage = (event: MessageEvent): void => {
      ...
      resolve({
        ...
        telemetry: {
          ...
          fileName: msg.telemetry.fileName,
          fileBytes: msg.telemetry.fileBytes,
          declaredPointCount: msg.telemetry.declaredPointCount,
          pointFormat: msg.telemetry.pointFormat,
          lazChunkCount: msg.telemetry.lazChunkCount,
          decodeStride: msg.telemetry.decodeStride,
          previewBudget: msg.telemetry.previewBudget,
          metadataBytes: msg.telemetry.metadataBytes,
          rangeRequests: msg.telemetry.rangeRequests,
          requestedBytes: msg.telemetry.requestedBytes,
          uniqueBytesRead: msg.telemetry.uniqueBytesRead,
          rereadBytes: msg.telemetry.rereadBytes,
          compressedBytesBeforePreview: msg.telemetry.compressedBytesBeforePreview,
