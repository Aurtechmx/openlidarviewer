...
import { productVerdict } from '../process/productVerdict';
...
const EXPORT_LOCATION: Readonly<Partial<Record<ProductId, string>>> = {
  contours: 'Analyse › Terrain › Contours (GeoJSON, DXF, SVG, map-sheet PDF)',
  dtm: 'Analyse › Terrain › Contours › DEM (ZIP)',
  dsm: 'Analyse › Terrain › Contours › DEM (ZIP)',
};
...
export class ProcessStudioPanel {
  ...
  update(facts: ScanFacts | null, view?: PreflightView): void {
    ...
    for (const p of PRODUCTS) {
      const { status: readiness, reason: verdict, preflight: pre } = productVerdict(facts, view, p.id);
