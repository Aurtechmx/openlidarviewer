...
export class AnalysePanel {
  ...
  /** The raster and relief preview tiles, rendered from the analysis result. */
  private readonly _surfaceTiles: SurfaceTiles;
  ...
  constructor(callbacks: AnalysePanelCallbacks = {}) {
    this._cb = callbacks;
    this._surfaceTiles = new SurfaceTiles({
      cb: callbacks,
      getResult: () => this._result,
      setConfidenceColorButton: (btn) => {
        this._confidenceColorBtn = btn;
  ...
  private _renderSurface(): void {
    ...
    if (!r?.surface) return;
    const mounted = this._surfaceTiles.render(r);
    this._surfaceRow.append(...mounted.elements);
    this._reliefRepaintCancel = mounted.cancelRepaint;
  }
