...
export class AnalysePanel {
  ...
  private _renderChips(): void {
    ...
    // Georeferencing is shown as ONE plain-language "anchored to the real
    // world?" element (glyph + headline + Position/Height facts) instead of the
    // old jargon "CRS / Datum · Unknown" chips, which meant nothing to a
    // non-surveyor. Generic — driven only by the two booleans + the real names.
    const g = georefStatus(q.crsKnown, q.datumKnown, {
      crsName: this._result!.dtm.crs,
      datumName: this._result!.dtm.verticalDatum,
    });
    const georef = el('div', { className: `olv-georef is-${g.tone}` });
    georef.setAttribute('aria-label', g.tooltip);
    this._hint(georef, g.tooltip);
    const glyph = el('span', { className: 'olv-georef-glyph' });
    // The SVG is generated by our own pure function from two booleans — no user
    // content — so assigning it as markup is safe.
    glyph.innerHTML = georefGlyphSvg(q.crsKnown, q.datumKnown);
    const text = el('div', { className: 'olv-georef-text' });
    text.append(
      el('span', { className: 'olv-georef-headline', text: g.headline }),
      el('span', { className: 'olv-georef-facts', text: `${g.positionLabel} · ${g.heightLabel}` }),
    );
    georef.append(glyph, text);
    this._chipsRow.append(georef);
