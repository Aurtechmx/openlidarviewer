...
export interface OverlayVertex {
  p: Vec3;
  /**
   * `pending` = the most recently placed point of an in-progress
   * measurement.
   * `snap-target` = the first vertex of a polygon draft that is
   *   ready to close — rendered with a pulsing cyan snap ring so the
...
export class MeasureOverlay {
  ...
  render(model: OverlayModel, camera: THREE.PerspectiveCamera, canvas: HTMLCanvasElement): void {
    ...
    // Vertices, with an invisible larger hit-circle for draggable
    // handles. Snap-target vertices (the first point of a closable
