...
export type ScanColourClass =
  ...
  | 'washed-out'
  /**
   * v0.3.10 formula-hardening pass — every sampled point is pure black
   * (all channels at 0). Auto-normalize used to silently classify this
   * as 'underexposed' and recommend a 1.6× exposure lift, which is
   * mathematically defined (lifting 0 gives 0) but useless and
   * confusing: the user sees a mystery exposure suggestion that does
   * nothing to the cloud. Surfacing the class explicitly lets the
   * Inspector show an honest "no colour data to normalize" message
   * instead of a fake suggestion.
   */
  | 'all-black';
...
export function rgbAutoNormalize(
  ...
  // v0.3.10 formula-hardening pass — all-black input check goes FIRST,
  // before the underexposed branch. Without it, the underexposed
  // branch would compute `exposure = min(1.6, 0.80 / max(0.01, 0))
  // = 1.6` and recommend a lift that does nothing (1.6 × 0 = 0). The
  // user would see a non-identity suggestion that has zero visible
  // effect, which reads as a bug. The explicit class lets the
  // Inspector say "no colour data to normalize" and skip the apply
  // button entirely. Threshold: p95 ≤ 1/255 catches both literally-
  // black input and inputs that round-trip through Uint8 quantization
  // to all zeros (1/255 ≈ 0.0039 is the smallest non-zero quantized
  // value, so anything below counts as effectively black).
  if (p95 <= 1 / 255 && p50 <= 1 / 255) {
    scanClass = 'all-black';
    reason =
      'All sampled points are black. No RGB data to normalize — ' +
      'apply would have no visible effect.';
    // settings stays at IDENTITY (no suggestion).
  } else if (p95 < 0.70 && p50 < 0.45) {
