...
export function densityForChunk(input: DensityInput): DensityColors {
  ...
  let cold = Math.max(0, input.minDensity ?? 0);

  // v0.3.10 formula-hardening pass — if the caller accidentally
  // passed `minDensity > maxDensity` (sliders dragged in the wrong
  // order, a preset with swapped bounds, etc.), the prior code
  // silently collapsed every point to the cold colour because
  // `Math.max(0, hot - cold) = 0` floored `logHot` to 0 and the
  // ternary fell through to `t = 0`. A user would see a flat-black
  // density overlay with no warning. Swap them here so the heatmap
  // still renders something useful from the same input range.
  // A single console.warn so a developer wiring sliders sees the
  // misuse, but no UI toast (this is a programmer-error path, not
  // user-visible).
  if (cold >= hot) {
    if (typeof console !== 'undefined' && console.warn) {
      console.warn(
        `densityColors: minDensity (${cold}) ≥ maxDensity (${hot}); ` +
          `swapping so the heatmap still renders. Check the caller.`,
      );
    }
    const tmp = cold;
    cold = Math.max(0, hot - 1e-9);
    hot = Math.max(1e-9, tmp);
  }
  ...
  // After the swap-guard above, `hot - cold > 0` is guaranteed, so
  // `logHot > 0` and the per-point ternary in the loop below never
  // falls through to 0 unconditionally. An empty cell (cellD = 0)
  // produces `log1p(max(0, 0 - cold)) = log1p(0) = 0` → t = 0 →
