...
export function applyRgbAppearance(
  ...
  //
  // v0.3.10 formula-hardening pass — the clamping below is intentional
  // and SILENT. A stray slider or preset that produces an out-of-range
  // value is bounded without warning:
  //   exposure  ≥ 0   — negative collapses to 0 (darkens to black)
  //   contrast  ≥ 0   — negative collapses to 0 (all pixels → 0.5)
  //   saturation ≥ 0  — negative collapses to 0 (fully desaturates)
  //   gamma     > 0   — non-positive falls back to 1 (identity)
  // The fast-path identity short-circuit below skips the loop when
  // every setting is at its neutral value, so the clamping cost is
  // zero in the common case. If a future control surface wants
  // user-visible warnings for clamped inputs, the right place is the
  // slider validation in `src/ui/Inspector.ts`, not here — keeping
  // this module pure-data so the test surface stays small.
  ...
  // Tint was originally ±15 % per channel — bumped to ±25 % so a full
  // tint drag produces the same magnitude of visual shift as a full
  // temperature drag (users reported tint "doesn't do anything" even
  // when the maths was running correctly — the gain was just too soft
  // to register against streaming COPC's tone-mapped surface).
  const tempGainR = 1 + 0.25 * temperature;
  const tempGainB = 1 - 0.25 * temperature;
  const tintGainR = 1 + 0.25 * tint;
  const tintGainG = 1 - 0.25 * tint;
  const tintGainB = 1 + 0.25 * tint;
